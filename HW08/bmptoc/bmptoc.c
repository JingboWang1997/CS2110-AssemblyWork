...
// ========================================================
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            unsigned int* bmpLocationRed = (i + j) * 32 + (1 * 8); //0 iteration: Red Address = 8
            unsigned int* bmpLocationGreen = (i + j) * 32 + (2 * 8); //0 iteration: Green Address = 16
            unsigned int* bmpLocationBlue = (i + j) * 32 + (3 * 8); //0 iteration: Blue Address = 24

            unsigned short* c_file = (i + j) * 16;
