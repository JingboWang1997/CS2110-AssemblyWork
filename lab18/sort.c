...
int sort_by_nameLength(const void *data1, const void *data2) {
	Person *p1 = (Person*) data1;
	Person *p2 = (Person*) data2;
	int diff = strlen(p1->name)-strlen(p2->name);
	if (diff != 0)
	{
		return diff;
	}
	...
	{
		return sort_by_name(data1, data2);
	}
...
int main(void) {
	...
	qsort(people, NUMPEOPLE, sizeof(Person), &sort_by_nameLength);
