...
list *deep_copy_list(list *listToCopy, list_copy copyFunc, list_op freeFunc)
{
  ...
  // a new list structure
  list *ptr = create_list();
  if (listToCopy == NULL || ptr == NULL || copyFunc == NULL || freeFunc == NULL)
  {
    free(ptr);
    return NULL;
  }
...
bool reverse(list *listToReverse)
{
    ...
    else if (listToReverse->size == 2)
    {
      node *head = listToReverse->head;
      node *tail = listToReverse->tail;
      head->next = NULL;
      tail->next = head;
    }
    else
